...
impl fmt::Display for Task {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let created_at = self.created_at.with_timezone(&Local).format(DEFAULT_TIME_FORMAT);
        // We indicate it is padded by 50 spaces
...
use std::io::{Error, ErrorKind}; // Include the `Error` type.
